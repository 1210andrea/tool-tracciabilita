...
-- Eseguire MANUALMENTE sul container PostgreSQL in fase di deploy
-- Comando: docker exec -i <nome_container_postgres> psql -U <utente> -d <database> < migration_magazzino_ricambi.sql
-- =============================================================
...
CREATE INDEX IF NOT EXISTS idx_reorders_spare_part
  ON reorders(spare_part_id);
CREATE INDEX IF NOT EXISTS idx_reorders_status
  ON reorders(status);
...
CREATE INDEX IF NOT EXISTS idx_movimenti_spare_part
  ON spare_parts_movimenti(spare_part_id);
CREATE INDEX IF NOT EXISTS idx_movimenti_tipo
  ON spare_parts_movimenti(tipo);
CREATE INDEX IF NOT EXISTS idx_movimenti_created_at
  ON spare_parts_movimenti(created_at);
