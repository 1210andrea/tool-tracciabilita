...
sparepartsRoutes.post('/spare-parts', authMiddleware, async (req, res, next) => {
  try {
    ...
    try {
      ...
      res.json({ item: { id: sparePartId, name: spR.rows[0].name, description: spR.rows[0].description, tipologie: cleanedTipologie } });
    } catch (e) {
...
sparepartsRoutes.put('/spare-parts/:id', authMiddleware, async (req, res, next) => {
  try {
    if (req.user?.role !== 'admin') return res.status(403).json({ error: 'Forbidden' });
    const { id } = req.params;
    const { name, description, tipologie } = req.body as { name?: string; description?: string; tipologie?: string[] };
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
...
// GET - restituisce problem_ids[] e problem_names[]
...
// POST nuova soluzione con problem_ids[]
...
// PUT aggiorna soluzione + problem_ids[]
