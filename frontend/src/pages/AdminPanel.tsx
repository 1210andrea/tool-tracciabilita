...
export default function AdminPanel() {
  ...
  const [sparePartForm, setSparePartForm] = useState<{ name: string; codice: string; tipologia: string[]; scorta_minima: number; quantita_riordino: number; description: string; quantita: number }>({ name: '', codice: '', tipologia: [], scorta_minima: 1, quantita_riordino: 10, description: '', quantita: 0 });
  ...
  const submitSparePart = async () => {
    try {
      ...
      const payload = {
        ...
        tipologia: sparePartForm.tipologia.length > 0 ? sparePartForm.tipologia.join(',') : null,
  ...
  const startEditSparePart = (part: SparePart) => {
    ...
    setSparePartForm({ name: part.name, codice: part.codice || '', tipologia: Array.isArray(part.tipologia) ? part.tipologia : (part.tipologia ? (part.tipologia as string).split(',').map((t: string) => t.trim()).filter(Boolean) : []), scorta_minima: part.scorta_minima ?? 1, quantita_riordino: part.quantita_riordino ?? 10, description: part.description || '', quantita: part.quantita ?? 0 });
  };
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-xs text-slate-400 mb-2">
                      Tipologie collegate
                      {sparePartForm.tipologia.length > 0 && (
                        <span className="ml-2 text-cyan-400 font-semibold">{sparePartForm.tipologia.length} selezionat{sparePartForm.tipologia.length === 1 ? 'a' : 'e'}</span>
                      )}
                    </label>
                    <div className="flex gap-2 mb-2 flex-wrap">
                      <button type="button" onClick={() => setSparePartForm((c) => ({ ...c, tipologia: [...availableTipologie] }))}
                        className="rounded-lg bg-slate-700 border border-slate-600 px-3 py-1.5 text-xs font-semibold text-slate-200 hover:bg-slate-600 transition">Tutte</button>
                      <button type="button" onClick={() => setSparePartForm((c) => ({ ...c, tipologia: availableTipologie.filter((t) => t.toUpperCase().startsWith('SR')) }))}
                        className="rounded-lg bg-slate-700 border border-slate-600 px-3 py-1.5 text-xs font-semibold text-slate-200 hover:bg-slate-600 transition">SR</button>
                      <button type="button" onClick={() => setSparePartForm((c) => ({ ...c, tipologia: availableTipologie.filter((t) => t.toUpperCase().startsWith('SIMM')) }))}
                        className="rounded-lg bg-slate-700 border border-slate-600 px-3 py-1.5 text-xs font-semibold text-slate-200 hover:bg-slate-600 transition">SIMM</button>
                      <button type="button" onClick={() => setSparePartForm((c) => ({ ...c, tipologia: [] }))}
                        className="rounded-lg bg-slate-700 border border-slate-600 px-3 py-1.5 text-xs font-semibold text-rose-400 hover:bg-slate-600 transition">Deseleziona tutto</button>
                    </div>
                    {availableTipologie.length === 0 ? (
                      <p className="text-xs text-slate-500 italic">Nessuna tipologia disponibile — aggiungi prima delle macchine.</p>
                    ) : (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 max-h-48 overflow-y-auto rounded-xl border border-slate-700 bg-slate-900/90 p-3">
                        {availableTipologie.map((tip) => (
                          <label key={tip} className="flex items-center gap-2 text-sm text-slate-200 cursor-pointer hover:text-white select-none">
                            <input type="checkbox" className="accent-cyan-500 h-4 w-4 cursor-pointer rounded"
                              checked={sparePartForm.tipologia.includes(tip)}
                              onChange={() => setSparePartForm((c) => ({
                                ...c,
                                tipologia: c.tipologia.includes(tip)
                                  ? c.tipologia.filter((t) => t !== tip)
                                  : [...c.tipologia, tip],
                              }))} />
                    ...
                    {editingUser?.id === u.id ? (
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {u.role === 'admin' ? (
  <span className="rounded-2xl border border-slate-700 px-4 py-2 text-xs font-semibold text-slate-500 cursor-not-allowed opacity-60">Admin</span>
) : (
