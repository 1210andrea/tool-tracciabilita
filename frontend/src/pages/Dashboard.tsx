...
// Task 5a + 6a: aggiunti case_number e operatori_list
type CaseItem = {
  id: string;
  machine_id: string;
  machine_code: string;
  ...
  case_number?: number;
  operatori_list?: { id: string; name: string }[];
};
...
export default function Dashboard() {
  ...
  // Task 5b: antepone #case_number alla label del caso
  const caseLabel = (item: CaseItem) =>
    [
      item.case_number ? `#${item.case_number}` : null,
      item.machine_code,
      item.problem_name !== 'N.D.' ? item.problem_name : null
    ].filter(Boolean).join(' · ') || item.machine_code;
                ...
                {cases.length === 0 ? (
                  ...
                  cases.map((item) => (
                    <tr key={item.id} className="transition hover:bg-slate-900/80">
                      {/* Task 5c: span con #case_number prima del machine_code */}
                      <td className="px-3 py-3 font-medium text-slate-100 sm:px-4 sm:py-4">
                        <span className="text-slate-500 font-mono text-xs mr-2">
                          {item.case_number ? `#${item.case_number}` : ''}
                        </span>
                        {item.machine_code}
                      </td>
                      {/* Task 6b: operatori_list come fonte primaria, fallback su operator_name */}
                      <td className="px-3 py-3 text-slate-300 sm:px-4 sm:py-4">
                        {(() => {
                          const ops = item.operatori_list as any[];
                          if (Array.isArray(ops) && ops.length > 0) {
                            return ops.map((o: any) => o.name).join(', ');
                          }
                          return item.operator_name ?? 'N.D.';
                        })()}
                      </td>
